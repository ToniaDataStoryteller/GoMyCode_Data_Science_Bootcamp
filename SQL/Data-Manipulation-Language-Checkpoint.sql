CREATE DATABASE QualiTech_Solutions
...
CREATE TABLE            CUSTOMERS (
   CUSTOMER_CODE        CHAR(5) PRIMARY KEY NOT NULL,
...
INSERT INTO PRODUCTS VALUES (4, 'Chef Anton Cajun Seasoning', 2, 2, '48 jars (6 ounces)', 110.00, 53, 0, 0);
INSERT INTO PRODUCTS VALUES (6, 'Grandma Boysenberry Spread', 3, 2, '12 jars (8 ounces)', 125.00, 120, 0, 0);
INSERT INTO PRODUCTS VALUES (7, 'Uncle Bob Organic Dried Pears', 3, 7, '12 cartons (1 kg)', 150.00, 15, 0, 0);
...
INSERT INTO PRODUCTS VALUES (20, 'Sir Rodney Marmalade', 8, 3, '30 jars', 405.00, 40, 0, 0);
INSERT INTO PRODUCTS VALUES (21, 'Sir Rodney Scones', 8, 3, '24 cartons x 4 pieces', 50.00, 3, 40, 0);
INSERT INTO PRODUCTS VALUES (22, 'Gustaf Knäckebröd', 9, 5, '24 cartons (500 g)', 105.00, 104, 0, 0);
...
INSERT INTO PRODUCTS VALUES (15, 'Genen Shouyu', 6, 2, '24 bottles (250 ml)', 78.00, 0, 0, -1)
;
...
--====================================================================================================================================
-- Display in descending order of seniority the male employees whose net salary (salary + commission) is greater than or equal
-- to 8000. The resulting table should include the following columns: Employee Number, First Name and Last Name, Age, and Seniority.                                             
--====================================================================================================================================
SELECT 
    EMPLOYEE_NUMBER,
    FIRST_NAME,
    LAST_NAME,
    DATEDIFF(YEAR, CONVERT(date, BIRTH_DATE, 103), GETDATE()) AS AGE,
    DATEDIFF(YEAR, CONVERT(date, HIRE_DATE, 103), GETDATE()) AS SENIORITY
FROM 
    EMPLOYEES
WHERE 
    TITLE = 'Mr.'
...
--================================================================================================================================================
-- Display products that meet the following criteria: (C1) quantity is packaged in bottle(s), (C2) the third character in the product name is
-- 't' or 'T', (C3) supplied by suppliers 1, 2, or 3, (C4) unit price ranges between 70 and 200, and (C5) units ordered are specified (not null).
...
--================================================================================================================================================
-- C1: List all products packaged in bottles
SELECT * FROM products
WHERE QUANTITY LIKE '%bottle%';

-- C2: List all products whose third character in the name is "t" or "T"
SELECT * FROM products
WHERE SUBSTRING(PRODUCT_NAME, 3, 1) IN ('t', 'T');

-- C3: List all products supplied by supplier number 1, 2, or 3
SELECT * FROM products
WHERE SUPPLIER_NUMBER IN (1, 2, 3);

-- C4: List all products with a unit price between 70 and 200 inclusive
SELECT * FROM products
WHERE UNIT_PRICE BETWEEN 70 AND 200;

-- C5: List all products whose UNITS_ON_ORDER is not null
SELECT * FROM products
...
--============================================================================================================================================
-- Display customers who reside in the same region as supplier 1, meaning they share the same country, city, and the last three digits of the 
-- postal code. The query should utilize a single subquery. The resulting table should include all columns from the customer table.                                                                                                                                                                           
--============================================================================================================================================
-- Query to find customers in same region as Supplier 1
SELECT c.*
FROM Customers c
WHERE 
...
-- 2. Display the message "apply old discount rate" if the order number is between 10000 and 10999, and "apply new discount rate" otherwise. The 
-- resulting table should display the columns: order number, new discount rate, and discount rate application note.  
--===============================================================================================================================================
WITH OrderTotals AS (
    SELECT 
        ORDER_NUMBER,
...
WHERE 
    -- Identify beverage products by packaging
    p.Category_Code LIKE '%bottle%' 
    OR p.Category_Code LIKE '%can%'
    OR p.Category_Code LIKE '%liter%'
    OR p.Category_Code LIKE '%cl%'
    OR p.Category_Code LIKE '%cc%'  -- For Chartreuse verte (750 cc)
    
    -- Identify by product name
    OR LOWER(p.Product_Name) LIKE '%ale%'
    OR LOWER(p.Product_Name) LIKE '%stout%'
    OR LOWER(p.Product_Name) LIKE '%beer%'
    OR LOWER(p.Product_Name) LIKE '%syrup%'
    OR LOWER(p.Product_Name) LIKE '%lager%'
    OR LOWER(p.Product_Name) LIKE '%cider%'
    OR LOWER(p.Product_Name) LIKE '%wine%'
    OR LOWER(p.Product_Name) LIKE '%spirit%'
    
    -- Specific beverage products from your data
    OR p.Product_Name IN ('Chang', 'Sasquatch Ale', 'Steeleye Stout', 
                         'Chartreuse verte', 'Laughing Lumberjack Lager', 
                         'Outback Lager', 'Lakkalikööri')
ORDER BY 
    s.Company;
...
HAVING COUNT(DISTINCT CASE WHEN CAT.CATEGORY_NAME = 'Desserts' THEN O.ORDER_NUMBER END) <= 1;

SELECT * FROM CUSTOMERS WHERE CUSTOMER_CODE = 'ALFKI';

SELECT COUNT(DISTINCT O.ORDER_NUMBER) AS DessertOrders
FROM CUSTOMERS C
...
--=================================================================================================================================================
-- Display customers who reside in France and the total amount of orders they placed every Monday in April 1998 (considering customers who haven't 
-- placed any orders yet). The resulting table should display the columns: customer number, company name, phone number, total amount, and country
--=================================================================================================================================================
SELECT 
    C.CUSTOMER_CODE,
    C.COMPANY,
...
LEFT JOIN ORDERS O
    ...
    AND TRY_CAST(O.ORDER_DATE AS DATE) BETWEEN '1998-04-01' AND '1998-04-30'
    AND DATEPART(WEEKDAY, TRY_CAST(O.ORDER_DATE AS DATE)) = 2  -- Monday (Assumes DATEFIRST = 7)
...
--=============================================================================================================================
-- Display for each customer from France the number of orders they have placed. The resulting table should display the columns:
...
--========================================================================================================================
-- Display the number of orders placed in 1996, the number of orders placed in 1997, and the difference between these two
-- numbers. The resulting table should display the columns: orders in 1996, orders in 1997, and Difference.
--========================================================================================================================
SELECT 
    SUM(CASE WHEN YEAR(CONVERT(date, ORDER_DATE, 3)) = 1996 THEN 1 ELSE 0 END) AS [Orders in 1996],
    SUM(CASE WHEN YEAR(CONVERT(date, ORDER_DATE, 3)) = 1997 THEN 1 ELSE 0 END) AS [Orders in 1997],
    SUM(CASE WHEN YEAR(CONVERT(date, ORDER_DATE, 3)) = 1997 THEN 1 ELSE 0 END) - 
    SUM(CASE WHEN YEAR(CONVERT(date, ORDER_DATE, 3)) = 1996 THEN 1 ELSE 0 END) AS [Difference]
FROM 
    ORDERS
WHERE 
    TRY_CONVERT(date, ORDER_DATE, 3) IS NOT NULL;
